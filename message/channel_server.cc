...
namespace {
void HandleConnection(sgx_enclave_id_t eid, int connd, WOLFSSL_CTX* ctx, Service* svc, std::atomic_int* status) {
...
// a quick solution to handle each connection in a thread. A proper thread pool will replace this impl later. 
...
struct ConnectionPool {
  ...
  void cleanup() {
    if (pool_) for (int i = 0; i < num_; i++) pool_.get()[i].close_conn();
  }
...
bool RatlsChannelServer::Serve() {
  ...
  printf("Waiting for a connction...\n");
    if ((connd = accept(sockfd_, (struct sockaddr*)&client_addr,
      &size)) == -1) {
...
int RatlsChannelServer::Close() {
  ...
  ctx_ = nullptr;
  int tmp = SGX_SUCCESS;
  if (enc_wolfSSL_Cleanup(id_, &tmp) != SGX_SUCCESS
    || tmp != SSL_SUCCESS) {
...
/* not used
// TODO: need to set cert and key for wolfssl
int NormalChannelServer::Initialize() {
  if (initialized_) {
    printf("NormalChannelServer reinit skipped\n");
    return 0;
  }
  // call base implementation
  if (ChannelServer::Initialize() == -1) {
    printf("NormalChannelServer failed to setup socket\n");
    return -1;
  }
  ...
  auto method = wolfTLSv1_2_server_method();
  if(!method) {
    printf("wolfTLSv1_2_server_method failure\n");
    return -1;
  }
...
int NormalChannelServer::Connect() {
  int connd; 
  struct sockaddr_in client_addr;
  socklen_t size = sizeof(client_addr);
#ifndef NDEBUG
  printf("sockfd: %d", sockfd_);
#endif // NDEBUG
  if ((connd = accept(sockfd_, (struct sockaddr*)&client_addr,
    &size)) == -1) {
    perror("ERROR: failed to accept the normal connection");
    return -1;
  }
  connd_ = connd; 
  ...
  if (wolfSSL_set_fd(ssl_, connd_) != SSL_SUCCESS) {
    printf("wolfSSL_set_fd failure\n");
    return -1;
  }
  connected_ = true;
...
int NormalChannelServer::CloseConnection() {
  if(ssl_) wolfSSL_free(ssl_);
  ssl_ = nullptr;
  if (connd_ != -1) close(connd_);
  connd_ = -1;
  connected_ = false;
  return 0;
}

int NormalChannelServer::Close() {
  CloseConnection();
  if (sockfd_ != -1) close(sockfd_);
  sockfd_ = -1;
  if (ctx_) wolfSSL_CTX_free(ctx_);
  ctx_ = nullptr;
  auto ret = wolfSSL_Cleanup();
  initialized_ = false;
  return (ret != SSL_SUCCESS) ? -1 : 0;
}

int NormalChannelServer::Read(std::string* output) {
  std::memset(read_buf(), 0, read_buf_size());
  auto read_size = wolfSSL_read(ssl_, read_buf(), read_buf_size()-1);
  if (read_size == 0) {
    printf("Server failed to read\n");
    return -1;
  }
  *output = std::string(read_buf(), read_size);
  return 0;
}

int NormalChannelServer::Send(const std::string& msg) {
  assert(msg.size() < INT_MAX);
  auto send_bytes = wolfSSL_write(ssl_, msg.data(),
    static_cast<int>(msg.size()));
  if (static_cast<uint32_t>(send_bytes) != msg.size()) {
