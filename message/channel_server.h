...
class ChannelServer {
 public:
  ChannelServer(uint16_t port, const CheckContext& check_ctx,
    ...
    : initialized_(false), serving_(false), checker_(check_ctx),
    svc_(nullptr), sockfd_(-1), port_(port),
...
class RatlsChannelServer : public ChannelServer {
 public:
  RatlsChannelServer(sgx_enclave_id_t id, uint16_t port,
    const CheckContext& check_ctx, bool enclave_client, int allowed_connection)
    : ChannelServer(port, check_ctx, allowed_connection), id_(id),
    enclave_client_(enclave_client), ctx_(nullptr), error_flag_(false) {}
