...
char* emem_getline(emem* m) {
  ...
  // advance m->curr
  m->curr += ((m->curr + to_copy) == (m->buf + m->size))
    ? to_copy : to_copy + 1; // skip the newline character if not at the end.
  return line;
}

size_t emem_read(void* dest, size_t size, emem* m) {
  size_t to_copy = ((m->curr + size) > (m->buf + m->size))
    ? (m->buf - m->curr) : size; 
  assert(dest != NULL);
