...
sgx_status_t seal_data(const char* aad_mac_text, uint32_t aad_mac_text_size, 
  ...
  uint32_t sealed_data_size = get_sealed_data_size(aad_mac_text_size,
    data_to_seal_size);
  ...
  sgx_status_t status = sgx_seal_data(
    aad_mac_text_size, (const uint8_t*) aad_mac_text,
...
sgx_status_t unseal_data(const uint8_t* sealed_data, uint32_t data_size,
  ...
  uint32_t mac_text_len = sgx_get_add_mac_txt_len(
  ...
  // is this checking really needed? i.e. decrypted length in each part must be smaller than the original sealed data length
  ...
  sgx_status_t status = sgx_unseal_data((const sgx_sealed_data_t*) sealed_data,
    de_mac_text, &mac_text_len, temp_unsealed_buf, &decrypt_data_len);
