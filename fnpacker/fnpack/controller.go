	...
	for i := range fpe.funcs {
		if !c.check_busy_or_exclusive(fpe, i, model_name) {
			...
			if fpe.in_use[model_name].get() != 1 {
				panic(errors.New("steal even already in use"))
			}
			target_id = i
			break
		}
...
func (c *FPController) PickFnInstance(name, model_name string) (string, uint64, error) {
	...
	if !found {
		return "", 0, errors.New("non-existent fgroup")
	}
	...
	if !found {
		...
		if target_id == -1 {
			...
			return "", 0, errors.New("Failed to allocate fn instance at create (All Busy!)")
		}
	...
	if last_in_use == 2 {
		...
		if last_in_use != 1 {
			panic(errors.New("in use just incremented should be > 1"))
		}
		target_id := c.steal_func(fpe, model_name)
		fpe.mapping[model_name] = target_id
		if target_id == -1 {
			fpe.in_use[model_name].decOne()
			return "", 0, errors.New("Failed to allcoate fn instance (All Busy)")
		}
		return createFnName(name, target_id), 0, nil
	}
}
...
func (c *FPController) ReleaseAndUpdateStats(name, model_name string, start_time, end_time time.Time, exec_time uint64) error {
	...
	fpe, found := c.fgroups[name]
	if !found {
		return errors.New("non-existent fgroup")
	}
	...
	if !found {
		fpe.mu.RUnlock()
		return errors.New(fmt.Sprintf("model %s not managed by %s", model_name, name))
	}
