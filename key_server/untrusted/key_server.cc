...
class ClientService : public Service {
 public:
  ClientService(sgx_enclave_id_t id, Store* store)
    : id_(id), store_(store) {}
...
class WorkerService : public Service {
 public:
  WorkerService(sgx_enclave_id_t id, Store* store)
    : id_(id), store_(store) {}
  inline int Handle(WOLFSSL* ssl) override {
    sgx_status_t retval = SGX_SUCCESS;
...
int main(int argc, char* argv[]) {
  ...
  std::thread client_service = std::thread(ServeClient, id, store,
    CON_CLIENT_CONNECT);
  std::thread worker_service = std::thread(ServeWorker, id, store,
    CON_WORKER_CONNECT);
