...
void ocall_save_kv(const void* key_buf, size_t key_len,
  const void* value_buf, size_t value_len, void* store_ptr) {
#ifndef NDEBUG
...
void ocall_get_kv(const void* key_buf, size_t key_len,
  void* value_buf, size_t value_buf_len, size_t* value_len,
  void* store_ptr) {
#ifndef NDEBUG
  ...
  std::string value;
  auto store = static_cast<Store*>(store_ptr);
  if (store->Get(CreateSealedFileName(
    std::string((const char*)key_buf, key_len)), &value) == -1) {
