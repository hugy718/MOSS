...
struct AccessListIterator {
  ...
  void next() {
    ...
    curr_ += curr_len_;
    auto next = memchr(curr_, '|', content_ + len_ - curr_);
    curr_len_ = (next == NULL) ? (content_ + len_ - curr_) 
      : ((char*) next - curr_);
  }
...
struct AccessList {
  AccessList(std::string* info) : info_(info) {
    assert(info);
  }

  AccessListIterator NewIterator() const {
    return AccessListIterator{info_->data(), info_->size(),
      false, NULL, 0};
  }

  void Append(const std::string& entry) {
    info_->append((info_->size() == 0)
      ? entry : ('|' + entry));
  }
...
struct RequestExecCtx {
  ...
  RequestExecCtx(sgx_status_t _sgxStatus, bool _request_failure,
    const std::string& _msg) : sgxStatus(_sgxStatus),
    request_failure(_request_failure), msg(_msg) {}
...
bool load_and_unseal_data(const std::string& key,
  std::string* output, void* store, sgx_status_t* status) {
  ...
  uint32_t unseal_buf_size = get_unsealed_data_size(
  ...
  *status = unseal_data((const uint8_t*) value_buf,
    static_cast<uint32_t>(retrieved_size), key.c_str(), key.size(),
    unseal_buf, unseal_buf_size);
...
sgx_status_t seal_and_save_data(const std::string& key, const std::string& value,
  void* store) {
  // use key that is file name as aad.
  int buf_size = get_sealed_data_size(
    static_cast<uint32_t>(key.size()),
    static_cast<uint32_t>(value.size()));
  uint8_t* seal_buf = (uint8_t*) malloc(buf_size);
  if (!seal_buf) return SGX_ERROR_OUT_OF_MEMORY;
  auto sgxStatus = seal_data(key.c_str(),
    static_cast<uint32_t>(key.size()), value.c_str(),
    static_cast<uint32_t>(value.size()), seal_buf, buf_size);
  if (sgxStatus == SGX_SUCCESS) {
    sgxStatus = ocall_save_kv(key.data(), key.size(), seal_buf,
      buf_size, store); 
  }
  free(seal_buf);
  return sgxStatus;
...
inline sgx_status_t gen_user_id(const std::string& id_key_bytes, std::string* uid) {
  sgx_sha256_hash_t output;
  auto status = sgx_sha256_msg(reinterpret_cast<const uint8_t*>(id_key_bytes.c_str()), id_key_bytes.size(), &output);
  if (status == SGX_SUCCESS) {
...
RequestExecCtx handle_user_register(const std::string& id_key, void* store) {
  // calculate the hash
  std::string user_id;
  auto id_key_bytes = base64_decode(
...
RequestExecCtx decrypt_payload(const std::string& user_id,
  const std::string& payload, void* store, std::string* output) {
  // get the id key
  std::string id_key;
  sgx_status_t status = SGX_SUCCESS;
  if(pass_table_.find(user_id) != pass_table_.end()) {
  ...
  // decrypt the buffer
  uint8_t* plain_txt;
  auto decode_bytes = base64_decode(reinterpret_cast<const uint8_t*>(payload.c_str()),
    payload.size());
  status = decrypt_content_with_key_aes(
    reinterpret_cast<const uint8_t*>(decode_bytes.c_str()), decode_bytes.size(),
    reinterpret_cast<const uint8_t*>(id_key.c_str()), &plain_txt);
  if (status != SGX_SUCCESS) {
    return {status, true, "failed to decrypt payload"};
  }
  output->assign(reinterpret_cast<const char*>(plain_txt),
    get_aes_decrypted_size(decode_bytes.size()));
  free(plain_txt);
  plain_txt = NULL;
...
sgx_status_t handle_upsert_decrypt_key(const std::string& record_key,
  const std::string& decrypt_key, void* store) {
...
static sgx_status_t handle_append_access_list(const std::string& record_key,
  const std::string& new_entry, void* store) {
...
int send_client_reply(WOLFSSL* ssl, const RequestExecCtx& ctx) {
  auto reply = (ctx.sgxStatus != SGX_SUCCESS)
    ? KeyServiceReply(false, "Request failed: SGX error: " + ctx.msg).EncodeTo()
    : ((ctx.request_failure)
      ? KeyServiceReply(false, "Request failed: " + ctx.msg).EncodeTo()
      : KeyServiceReply(true, "Request success").EncodeTo()
    );
...
sgx_status_t enc_client_service(WOLFSSL* ssl, void* store) {
  ...
  KeyServiceRequest req = DecodeKeyServiceRequest(
    std::string(user_request_buf, strlen(user_request_buf)));
  if (req.type_ == USER_REGISTER) {
...
std::string extract_mr_enclave(WOLFSSL* ssl) {
  // TODO needs to augment the ratls library with function that extracts mrenclave from report / simply reply report.
  // here we use the testing mrenclave value
...
#ifndef NDEBUG
    ocall_debug_print_string(std::string("checking access list for: " + entry).c_str());
#endif // NDEBUG
...
inline int send_worker_failure_reply(WOLFSSL* ssl,
  ...
  auto reply = KeyServiceWorkerReply(std::move(error_msg)).EncodeTo();
  assert(reply.size() < INT_MAX);
  wolfSSL_write(ssl, reply.data(), static_cast<int>(reply.size()));
  return 0;
}
...
bool check_permission_in_access_list(const std::string& acl_id,
  const std::string& candidate, void* store, sgx_status_t* sgxStatus) {
  std::string acl;
  if (cache_.find(acl_id) != cache_.end()) acl = cache_[acl_id];
...
sgx_status_t enc_worker_service(WOLFSSL* ssl, void* store) {
  ...
  GetKeyRequest req = GetKeyRequest::DecodeFrom(
  ...
  // send reply 
  auto reply = KeyServiceWorkerReply(req.user_id(), user_decrypt_key,
    req.model_id(), model_decrypt_key).EncodeTo();
#ifndef NDEBUG
