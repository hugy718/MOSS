...
void ocall_load_content(const char* key, size_t len,
  char** value, size_t* vlen, void* store) {
  {
  ...
  size_t loaded_len = 0;
  char* loaded = reinterpret_cast<Store*>(store)->Get(model_name,
    &loaded_len);
  if (!loaded) {
...
int Worker::Execute(const WorkerRequest& request, std::string* output) {
...
#endif // NDEBUG
  enc_model_inference(eid_, &retval,
    request.user_id_.data(), request.user_id_.size(),
    request.encrypted_sample_.data(), request.encrypted_sample_.size(),
    request.model_name_.data(), request.model_name_.size(),
    (void*) model_store_, request.key_service_address_.data(),
    request.key_service_address_.size(), request.key_service_port_,
    &output_size);
  ...
  if (output_buf == NULL) {
    printf("malloc failed for prediction result buffer %d\n", output_size);
    return -1;
  } 
 ...
 */
bool Worker::Handle(uint64_t handle_id, const std::string& sample_request,
  std::string* output) {
  ...
  WorkerRequest request = DecodeWorkerRequest(sample_request);
  ...
  if (ret == 0) {
    output->reserve(msg_prefix.length() + strlen(hex_result));
    output->assign(std::move(msg_prefix));
    output->append(hex_result, strlen(hex_result));
  // } else *output = msg_prefix + "prediction error";
  } else *output = msg_prefix + prediction;
