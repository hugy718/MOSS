...
int main(int argc, char* argv[]) {
  ...
  if (argc == 3) {
    ...
    if (ReadFileToString(sample_request_path, &sample_request) == -1) {
...
for (int j = 0; j < 2; j++) {
  ...
  Worker worker(WORKER_ENCLAVE_FILENAME,
    std::unique_ptr<Store>(OpenFsStore(
  ...
  for (uint64_t i = 0; i < INFERENCE_COUNT; i ++) {
    handlers.emplace_back(&Worker::Handle, &worker, i, sample_request,
      &outputs[i]);
  }
  for (auto& h : handlers) {
    h.join();
