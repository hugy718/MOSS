...
class Worker {
 public: 
  Worker(const std::string& enclave_file_name,
    const std::string& remote_store_path, const std::string& local_store_path)
    : initialize_(false), closed_(false), enclave_file_name_(std::move(enclave_file_name)),
    eid_(0), model_store_(OpenFsStore(std::move(remote_store_path),
      std::move(local_store_path))) {}

  Worker(const std::string& enclave_file_name, 
    std::unique_ptr<Store> model_store)
    : initialize_(false), enclave_file_name_(std::move(enclave_file_name)),
    eid_(0), model_store_(model_store.release()) {}
