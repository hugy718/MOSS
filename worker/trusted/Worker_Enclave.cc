...
SimpleCache<ModelContext> model_cache{1};
// for both model and user key
SimpleCache<KeySet> key_cache{16};
// shared ra channel
RatlsChannelClient* channel = nullptr;
...
int decode_decryption_keys(const std::string& ks_response) {
  ...
  int ret = DecodeKeyServiceWorkerReply(ks_response,
    &user_id, &input_key_b64, &model_id, &model_key_b64);

  // base64 decode the keys
  if (ret != 0) return ret;
  auto input_key_bytes = base64_decode((const uint8_t*) input_key_b64.data(),
    input_key_b64.size());
  auto model_key_bytes = base64_decode((const uint8_t*) model_key_b64.data(),
    model_key_b64.size());
  ...
  key_cache.AddToCache(GetKeyCacheKey(), std::unique_ptr<KeySet>(
    new KeySet{std::move(input_key_bytes), std::move(model_key_bytes)}));
...
sgx_status_t fetch_from_ks(const std::string& request,
  const std::string& ks_addr, uint16_t ks_port, std::string* ks_response) {
  assert(ks_response);
  if (!channel) {
    channel = new RatlsChannelClient{ks_port, std::move(ks_addr), CheckContext{}};
    if (channel->Initialize() == -1) {
      printf("failed to setup ratls ctx\n");
      return SGX_ERROR_SERVICE_UNAVAILABLE;
    }
  }
  // RatlsChannelClient channel{ks_port, std::move(ks_addr), CheckContext{}};
  
  // if ((channel.Initialize() == -1) || (channel.Connect() == -1)) {
...
sgx_status_t enc_model_inference(
  const char* user_id, size_t user_id_size,
  const char* input_data, size_t input_data_size,
  const char* model_id, size_t model_id_size, void* store,
  const char* ks_addr, size_t addr_len, uint16_t ks_port,
  size_t* output_size) {

// only allow single permitted model under equal_exec_time setting
#if EQUAL_EXEC_TIME
  if ((sizeof(PERMITTED_MODEL)-1 != model_id_size) || (memcmp(PERMITTED_MODEL, model_id, model_id_size) != 0)) {
  ...
  // checking of input size
  if (input_data_size != get_aes_encrypted_size(INPUT_BUF_SZ)) {
    return SGX_ERROR_INVALID_PARAMETER;
  }
  ...
  if (cache_ret == 0) {
    ...
    auto create_key_request_msg = [&]() -> std::string {
      return GetKeyRequest(std::string(user_id, user_id_size),
          std::string(model_id, model_id_size)).EncodeTo();
    }; 
    auto status = fetch_from_ks(create_key_request_msg(),
      std::string(ks_addr, addr_len), ks_port, &ks_response);
  ...
  unsigned char* decrypt_input = NULL;
  {
    ...
    auto ret = decrypt_content_with_key_aes((const uint8_t*) input_data,
      input_data_size, (const uint8_t*) input_key_buf, &decrypt_input);
    if (ret != SGX_SUCCESS) {
      ClearRequestContext();
  ...
  ModelContext* decrypt_model = nullptr;
#if EQUAL_EXEC_TIME
  if (!initialized_once) {
  ...
  if (cache_ret == 0) {
    ...
    auto model_context = load_model(model_id_to_use, model_id_to_use_len,
      model_key_buf, store, &ret);
    if(!model_context) {
      ClearRequestContext();
      return ret;
    }
    decrypt_model = model_cache.AddToCache(GetModelCacheKey(),
  ...
  ret = execute_inference(
    (char*)decrypt_input, get_aes_decrypted_size(input_data_size),
    *decrypt_model, model_rt, prediction_buf, OUTPUT_BUF_SZ, &prediction_size);
  ...
  *output_size = (ret == SGX_SUCCESS) 
    ? get_aes_encrypted_size(OUTPUT_BUF_SZ) : 0;
...
sgx_status_t enc_get_encrypted_prediction(uint8_t* prediction,
  size_t size) {
  size_t whole_cipher_text_size = get_aes_encrypted_size(OUTPUT_BUF_SZ);

  // check if enough space in untrusted memory
  if ((sgx_is_outside_enclave(prediction, size) != 1)
    && (size < whole_cipher_text_size)) return SGX_ERROR_UNEXPECTED;

  // encryption
  unsigned char* output = NULL;
  sgx_status_t ret = encrypt_content_with_key_aes( 
      (const uint8_t*) prediction_buf, OUTPUT_BUF_SZ, 
      (const uint8_t*) input_key_buf, &output);
  if (ret == SGX_SUCCESS) memcpy(prediction, output,
    whole_cipher_text_size); 
  free(output);
