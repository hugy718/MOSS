...
#ifdef USE_WORKER_SERVICE
  #include "worker_service.h"
#else
  #include "sample_service.h"
#endif // USE_WORKER_SERVICE

// pick platform messenger (currently only openwhisk)
#include "ow_messenger.h"
...
// pick platform messenger
OwMessenger msgr_impl;
Messenger* msgr = &msgr_impl;
...
void on_work(uv_work_t* req) {
  ...
  if ((client->task_type != INIT) || (code != OK)) {
...
#ifdef USE_WORKER_SERVICE
  if (argc != 4) {
    fprintf(stderr, "Usage: worker_enclave, remote_store, local_store\n");
    exit(1);
  }
  auto enclave_path = argv[1];
  auto remote_store_path = argv[2];
  auto local_store_path = argv[3];
  WorkerService s{enclave_path, remote_store_path, local_store_path};
#else
  (void) argv;
  (void) argc;
  SampleService s;
