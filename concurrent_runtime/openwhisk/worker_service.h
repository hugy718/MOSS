...
class WorkerService : public Service {
 public:
  ...
  WorkerService(const std::string& enclave_path,
    const std::string& filestore_path, const std::string& store_path)
    // : worker_(std::move(enclave_path), std::move(filestore_path),
    //   std::move(store_path)) {
    : worker_(std::move(enclave_path), std::unique_ptr<Store>(OpenFsStore(filestore_path, "", 0))) {
      printf("Worker service config:\n enclave file name: %s\n\
        store path: %s, filestore path %s\n", enclave_path.c_str(),
        store_path.c_str(), filestore_path.c_str());
    }
