...
class ModelRTImpl : public ModelRT {
 public:
  ModelRTImpl() : model_(nullptr), micro_error_reporter_(),
    interpreter_(nullptr), tensor_arena_(nullptr) {}
  ~ModelRTImpl() {
  ...
  sgx_status_t maybe_init(const char* model_buf) {
    if (model_ != model_buf) {
      ...
      const tflite::Model* model = tflite::GetModel(
        reinterpret_cast<const int8_t*>(model_buf));
      if (model->version()!= TFLITE_SCHEMA_VERSION) {
#ifndef NDEBUG
        printf("Model provided is schema version %d not equal to \
      ...
      interpreter_.reset(new tflite::MicroInterpreter(model, resolver_,
        tensor_arena_, kTensorArenaSize, &micro_error_reporter_));
 ...
 private:
  ...
  tflite::AllOpsResolver resolver_; // resolver lifetime needs to be longer than interpreter.
...
std::unique_ptr<ModelContext> load_model(const char* model_id, size_t model_id_len, 
  const char* dec_key, void* store, sgx_status_t* status) {
  assert(status);
  assert(dec_key);
  assert(model_id);
  ...
  *status = decrypt_content_with_key_aes(
    (const uint8_t*) cipher_text, load_len, 
    (const uint8_t*) dec_key, &content); 
  free(cipher_text);
  cipher_text = nullptr;
  if (*status != SGX_SUCCESS) return NULL;  // note that the caller only return a buffer if success. 
...
sgx_status_t execute_inference(
  const char* input_src, size_t input_src_size,
  const ModelContext& model_context, ModelRT* model_rt, char* output,
