...
TVM_DLL void* tvm_runtime_create(const char* json_data,
  const char* params_data, const uint64_t params_size,
  uint8_t* crt_memory_buf, size_t crt_memory_size) {
  ...
  // get pointers
  TVM_CCALL(PageMemoryManagerCreate(&g_memory_manager,
    crt_memory_buf, crt_memory_size, CRT_MEMORY_PAGE_SIZE_LOG2));
