...
class ModelContextImpl : public ModelContext {
 public:
  ModelContextImpl(char* graph, size_t graph_len, char* params_data,
    size_t params_len) : graph_(graph), graph_len_(graph_len),
    params_data_(params_data), params_len_(params_len) {}
  ~ModelContextImpl() {
...
class ModelRTImpl : public ModelRT {
 public:
  ModelRTImpl(uint8_t* buf, void* handle) : tvm_memory_buffer_(buf),
    handle_(handle) {}
...
ModelRT* model_rt_init (const ModelContext& ctx, sgx_status_t* status) {
  ...
  void* handle = tvm_runtime_create(mc.graph(), mc.params_data(), mc.params_len(),
    buf, TVM_CRT_MEMORY_BUFFER_SZ);
  ocall_print_time();
...
size_t load_and_decode (const char* id, size_t len, const char* dec_key,
  ...
  *status = decrypt_content_with_key_aes(
    (const uint8_t*) cipher_text, loaded_len,
...
std::unique_ptr<ModelContext> load_model(const char* model_id,
  size_t model_id_len, const char* dec_key, void* store,
  sgx_status_t* status) {
  assert(status);
  assert(dec_key);
  assert(model_id);
  ...
  memcpy(fetch_id+model_id_len, graph_file_suffix,
    sizeof(graph_file_suffix)); 
  char* graph = nullptr;
  size_t graph_len = load_and_decode(fetch_id,
    model_id_len+sizeof(graph_file_suffix), dec_key, store, &graph, status);
  if (!graph) return nullptr;

  memcpy(fetch_id+model_id_len, params_file_suffix,
    sizeof(params_file_suffix));
  char* params = nullptr;
  size_t params_len = load_and_decode(fetch_id,
    model_id_len+sizeof(params_file_suffix), dec_key, store, &params, status);
  if (!params) {
    free(graph);
    return nullptr;
  }
...
sgx_status_t execute_inference(
  ...
  const ModelContext& /*ctx*/, ModelRT* model_rt, char* output_buf,
